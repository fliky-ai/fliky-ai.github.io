...
// ============ ИСПРАВЛЕННОЕ РЕДАКТИРОВАНИЕ ПРОФИЛЯ ============
...
function editName() {
    ...
    showModal({
        ...
        console.log('📝 Результат модалки (имя):', newName);
        if (newName !== null && newName.trim()) {
            const name = newName.trim();
            console.log('📤 Отправка update_profile (имя):', name);
            
            socket.emit('update_profile', { name: name }, (response) => {
                console.log('📨 Ответ сервера (имя):', response);
                ...
                if (response && response.status === 'ok') {
                    ...
                    console.error('❌ Ошибка сохранения имени:', response);
...
function editUsername() {
    ...
    showModal({
        ...
        subtitle: 'Можно использовать символы a-z, 0-9 и подчёркивания. Минимальная длина 5 символов.',
        ...
        if (newUsername !== null && newUsername.trim()) {
            ...
            // Валидация на клиенте (как в Telegram)
            if (!/^[a-z0-9_]{5,32}$/.test(username)) {
                showAlert('❌ Username должен содержать только a-z, 0-9, _ и быть не короче 5 символов');
                return;
            }
            
            socket.emit('check_username', { username: username }, (response) => {
                console.log('📨 Ответ check_username:', response);
                ...
                socket.emit('update_profile', { username: username }, (response) => {
                    console.log('📨 Ответ update_profile (username):', response);
                    ...
                    if (response && response.status === 'ok') {
                        ...
                        console.error('❌ Ошибка сохранения username:', response);
                        showAlert('❌ Ошибка: ' + (response?.message || 'Не удалось сохранить username'));
                    }
                });
            });
        }
    ...
    showModal({
        ...
        if (newBio !== null) {
            ...
            socket.emit('update_profile', { bio: bio }, (response) => {
                console.log('📨 Ответ сервера (bio):', response);
                ...
                if (response && response.status === 'ok') {
                    ...
                    console.error('❌ Ошибка сохранения bio:', response);
                    showAlert('❌ Ошибка: ' + (response?.message || 'Не удалось сохранить описание'));
                }
            });
        }
    });
...
function changeAvatar() {
    ...
    showModal({
        ...
        if (url !== null && url.trim()) {
            ...
            socket.emit('update_profile', { photo_url: avatarUrl }, (response) => {
                console.log('📨 Ответ сервера (avatar):', response);
                ...
                if (response && response.status === 'ok') {
                    ...
                    console.error('❌ Ошибка сохранения аватара:', response);
                    showAlert('❌ Ошибка: ' + (response?.message || 'Не удалось сохранить аватар'));
                }
            });
        }
    });
